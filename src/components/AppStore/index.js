...
class AppStore extends Component {
  state = {searchInput: '', moduleOpt: tabsList[0].tabId}
  typing = event => {
    this.setState({
  ...
  render() {
    const {searchInput, moduleOpt} = this.state
    const moduleWise = appsList.filter(echmod => echmod.category === moduleOpt)
    const searchResults = moduleWise.filter(echResult =>
      echResult.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return (
      <div className="home">
        ...
        <ul className="listo">
          {tabsList.map(eachItem => (
            <TabItem
              ...
              tabItemsList={eachItem}
            />
          ))}
        </ul>
        <ul className="listo-2">
          {searchResults.map(eachList => (
            <AppItem key={eachList.appId} allAppList={eachList} />
          ))}
        </ul>
      </div>
    )
